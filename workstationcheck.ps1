...
function Check-SecuritySettings {
    ...
    try {
        ...
        if (-not $uacSettings.PromptOnSecureDesktop) {
            $recommendations += @(
                ...
                "Activer l'invite du bureau sécurisé"
            )
        }
        if ($uacSettings.ConsentPromptBehaviorAdmin -notin (2,5)) {
            $recommendations += @(
                "Set admin prompt behavior to 2 (consent) or 5 (consent for non-Windows)",
                "Définir le comportement d'invite administrateur sur 2 (consentement) ou 5 (consentement pour les programmes non-Windows)"
            )
        }
        if ($uacSettings.ConsentPromptBehaviorUser -ne 3) {
            $recommendations += @(
                "Set standard user behavior to 3 (require admin credentials)",
                "Définir le comportement utilisateur standard sur 3 (demander les informations d'identification administrateur)"
            )
        }
    ...
    try {
        ...
        if ($powerScheme -match "Power Scheme GUID: (.+) \((.+)\)") {
            $schemeGuid = $matches[1].Trim()
            $schemeName = $matches[2].Trim()
        } else {
            throw "Could not parse power scheme"
        }
        ...
        try {
            ...
            powercfg /query $schemeGuid SUB_VIDEO VIDEOIDLE | Out-Null
        } catch {
            $canModifyPower = $false
        }
        ...
        # Get current timeout values with error handling
        try {
            ...
            # Parse AC values
            if ($videoSettings -match "Current AC Power Setting Index: 0x([0-9a-fA-F]+)") {
                $screenACValue = [math]::Round([Convert]::ToInt32($matches[1], 16) / 60)
            } else {
                $screenACValue = "Not configured"
            }

            # Parse DC values
            if ($videoSettings -match "Current DC Power Setting Index: 0x([0-9a-fA-F]+)") {
                $screenDCValue = [math]::Round([Convert]::ToInt32($matches[1], 16) / 60)
            } else {
                $screenDCValue = "Not configured"
            }

            # Parse Sleep AC values
            if ($sleepSettings -match "Current AC Power Setting Index: 0x([0-9a-fA-F]+)") {
                $sleepACValue = [math]::Round([Convert]::ToInt32($matches[1], 16) / 60)
            } else {
                $sleepACValue = "Not configured"
            }

            # Parse Sleep DC values
            if ($sleepSettings -match "Current DC Power Setting Index: 0x([0-9a-fA-F]+)") {
                $sleepDCValue = [math]::Round([Convert]::ToInt32($matches[1], 16) / 60)
            } else {
                $sleepDCValue = "Not configured"
            }
